...
export function Buttons({ strAssignments }: Props) {
  ...
  const linkTw = "transition-colors flex flex-col gap-y-1 items-center duration-300";
  ...
  return (
    <div className="relative grid grid-cols-4 text-center w-full max-w-sm text-sm min-w-[256px]">
      <button onClick={() => handleClick("all")} className={cn(linkTw, filter === "all" ? "text-blue-500" : "text-muted-foreground")}>
        <p className={cn(countTw, filter === "all" && activeMap[filter].split(" ")[0])}>{totalAssignments}</p>
        <p>Alle</p>
      </button>
      <button onClick={() => handleClick("submitted")} className={cn(linkTw, filter === "submitted" ? "text-green-500" : "text-muted-foreground")}>
        <p className={cn(countTw, filter === "submitted" && activeMap[filter].split(" ")[0])}>{submittedAssignments}</p>
        <p>Afleveret</p>
      </button>
      <button onClick={() => handleClick("pending")} className={cn(linkTw, filter === "pending" ? "text-yellow-500" : "text-muted-foreground")}>
        <p className={cn(countTw, filter === "pending" && activeMap[filter].split(" ")[0])}>{pendingAssignments}</p>
        <p>Venter</p>
      </button>
      <button onClick={() => handleClick("missing")} className={cn(linkTw, filter === "missing" ? "text-red-500" : "text-muted-foreground")}>
        <p className={cn(countTw, filter === "missing" && activeMap[filter].split(" ")[0])}>{missingAssignments}</p>
        <p>Mangler</p>
      </button>
      <div className={cn("absolute -bottom-2 h-0.5 rounded-md transition-all duration-300 w-1/4", activeMap[filter])} />
    </div>
  );
}

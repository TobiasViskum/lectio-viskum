...
export function getFormattedDate(assignment: Assignment) {
  ...
  const hoursBetween = Math.floor((msBetween % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)) - 1;
  const minutesBetween = Math.floor((msBetween % (1000 * 60 * 60)) / (1000 * 60));
  ...
  if (daysBetween > 14 || daysBetween < 0) {
    ...
    return [Math.floor(daysBetween), daysText, "og", hoursBetween, hoursText, secondsBetween].join(" ");
  } else if (hoursBetween > 1) {
    return [hoursBetween, hoursText, "og", minutesBetween, minutesText].join(" ");
  } else if (minutesBetween > 1) {
    return [minutesBetween, minutesText, "og", secondsBetween, secondsText].join(" ");
  } else {

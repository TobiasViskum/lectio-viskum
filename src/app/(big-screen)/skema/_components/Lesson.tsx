...
type Props = { lesson: Lesson; timestamps: number[]; lessonsAddedAtTimestamp: { start: number; end: number; totalAdded: number }[] };

export async function Lesson({ lesson, timestamps, lessonsAddedAtTimestamp }: Props) {
  ...
  if (foundLesson) {
    leftOffset = `calc((var(--lesson-width) / ${lesson.overlappingLessons + 1}) * ${foundLesson.totalAdded})`;
  ...
  return (
    <Link key={Math.random()} href={"/skema"} className={cn(`group hover:scale-[1.025] bg-opacity-50 absolute w-full rounded-md transition-[transform,_background-color] hover:bg-opacity-90 flex gap-x-2 hover:z-50 overflow-hidden`, backgroundColor)} style={{ top: `calc(var(--offset-top-lesson) + var(--height-hour) * ${multi})`, height: `calc((${endTime - startTime}) * var(--height-hour))`, width: `calc(var(--lesson-width) / ${lesson.overlappingLessons + 1})`, left: leftOffset }}>
      ...
      <div className="w-full overflow-hidden">
        ...
        <div className="pt-1 overflow-auto break-words w-auto">
          <p className={cn("py-0.5 font-semibold transition-colors leading-3", textColor, hoverTextColor, titleSize)}>{lesson.subjects.join(", ")}</p>
          <p className={cn(isSubjectsEmpty ? `font-semibold transition-colors leading-3 ${titleSize}` : "text-xs transition-colors w-full", textColor, hoverTextColor)}>{lesson.title}</p>
          {!displayOnlyTitle && (
            <>
              <p className={cn("text-xs transition-colors", textColor, hoverTextColor)}>{lesson.classrooms.join(", ")}</p>
              <p className={cn("transition-colors", textColor, hoverTextColor, lesson.classes.join(", ").length > 40 ? "text-2xs" : "text-xs")}>{lesson.classes.join(", ")}</p>
            </>
          )}
        </div>
      </div>
